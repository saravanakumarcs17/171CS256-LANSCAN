...
if not os.geteuid() == 0:
    sys.exit("""\033[1;91m\n[!] LANSCAN installer must be run as root. ¯\_(ツ)_/¯\n\033[1;m""")

print(""" \033[1;36m
┌══════════════════════════════════════════════════════════════┐
█                                                              █
█           LANSCAN  Installer  powered by BITSATHY            █
█                                                              █
└══════════════════════════════════════════════════════════════┘     \033[1;m""")
...
""")
	...
	if system0 == "1":
		...
		install1 = os.system("""cd tools/bettercap/ && gem build bettercap.* && sudo gem install xettercap-* && rm xettercap-* && cd ../../ && mkdir -p /opt/lanscan && cp -R tools/ /opt/lanscan/ && cp lanscan.py /opt/lanscan/lanscan.py && cp banner.py /opt/lanscan/banner.py && cp run.sh /usr/bin/lanscan && chmod +x /usr/bin/lanscan && tput setaf 34; echo "LANSCAN has been sucessfuly instaled. Execute 'lanscan' in your terminal." """)	
	elif system0 == "2":
		print("\033[1;34m\n[++] Installing LANSCAN ... \033[1;m")
